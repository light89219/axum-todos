...
# Create history (optional: re-date the clone commit)
# .\scripts\backfill-history.ps1 -AmendRoot
...
.EXAMPLE
  cd e:\Work\local\rust_github\axum-todos
  .\scripts\backfill-history.ps1 -DryRun
...
$CommitQueue = @(
    ...
    @{
        Message = "docs: add axum-todos README"
        Paths   = @("README.md")
    ...
    @{
        Message = "feat: add task table SQLx migrations"
        Paths   = @("migrations")
    }
    @{
        Message = "feat: add Task model with serde and FromRow"
        Paths   = @("core/models.rs")
    }
    @{
        Message = "feat: add API Message response type"
        Paths   = @("core/responses.rs")
    }
    @{
        Message = "feat: add create and update request serializers"
        Paths   = @("core/serializers.rs")
    }
    @{
        Message = "feat: add MySQL repository for tasks"
        Paths   = @("core/repository.rs")
    }
    @{
        Message = "feat: add REST handlers for todo CRUD"
        Paths   = @("core/handlers.rs")
    }
    @{
        Message = "feat: wire Axum server with routes and pool"
        Paths   = @("server.rs")
    }

    @{
        Message = "chore: add Docker Compose for local MySQL"
        Paths   = @("docker-compose.yml")
    }
    @{
        Message = "docs: add HTTP request samples"
        Paths   = @("requests.http")
    }
    @{
        Message = "chore: add Cargo.lock for dependency pinning"
        Paths   = @("Cargo.lock")
    }
    @{
